...
def load_tools_from_string(tool_string):
    # create a dictionary to store the tools temporarily
    tools = {}

    # Parse the module name from the filename
    module_name = "temp_tool_module"

    # Load the module
    try:
        spec = importlib.util.spec_from_loader(module_name, loader=None)
        module = importlib.util.module_from_spec(spec)
        exec(tool_string, module.__dict__)
        sys.modules[module_name] = module
    except Exception as e:
        print(f"Error loading module {module_name}: {e}")
        # return a dictionary with the module name and a detailed error message
...
if __name__ == "__main__":
    ...
    # test loading a tool from a string
    tool_string = """
def test_tool(param1, param2):
    ...
    """
    tools = load_tools_from_string(tool_string)
    tool_query = {"actionName": "test_tool", "param1": "Hello", "param2": "World"}
    tool_return = execute_tool(tool_query, tools)
    print(tool_return)
